...
auto FileDialog::openFile(const std::string& iFilter) -> std::filesystem::path {
	...
	const std::string tmp = app::Application::get().getAssetDirectories().front().assetsPath.string();
...
auto FileDialog::saveFile([[maybe_unused]] const std::string& iFilter, const std::string& iDefaultName)
	...
	const std::string& filters{iFilter};
	const auto ff = parseFilter(filters);
	const std::string defaultDir = app::Application::get().getAssetDirectories().front().assetsPath.string();
