...
/**
 * @brief Holds the list of open documents and tracks the active one.
 *
...
public:
	...
	/// @brief Add a document to the manager. Takes ownership. Returns a stable pointer.
	auto add(uniq<Document> iDocument) -> Document*;

	/// @brief Remove the document with the given id. Returns true if it was removed.
	auto remove(core::UUID iId) -> bool;

	/// @brief Remove and destroy every document (called on project close / editor detach).
	void clear();

	/// @brief Make the document with this id the active one. No-op if not found.
	void setActive(core::UUID iId);

	/// @brief Directly mark the given document as active (pointer must belong to this manager).
	void setActive(Document* iDocument);

	/// @brief The currently active document, or nullptr if none.
	[[nodiscard]] auto getActive() const -> Document* { return mp_active; }

	/// @brief Lookup by id. Returns nullptr if not found.
	[[nodiscard]] auto find(core::UUID iId) const -> Document*;

	/// @brief Access the full list of open documents.
	...
	/// @brief True when no document is open.
