...
/// @brief Inline span kind emitted by the Markdown parser.
...
/// @brief One inline span emitted by the parser.
struct MdInline {
...
/// @brief Per-column horizontal alignment in a GFM table.
...
/// @brief Standalone block image (a paragraph that contains only an image).
struct MdImage {
...
/// @brief GFM-style table (head row + N body rows). Cells are inline-span sequences.
struct MdTable {
...
/// @brief Ordered or unordered list. Each item is itself a list of blocks.
struct MdList {
	bool ordered = false;                    ///< `true` for `1. … 2. …`, `false` for `- … - …`.
	std::vector<std::vector<MdBlock>> items; ///< One vector of blocks per list item.
};

/// @brief Block quote, body is the nested block sequence.
struct MdBlockQuote {
	std::vector<MdBlock> body;///< Quoted blocks (typically paragraphs).
};

/// @brief Top-level block variant produced by the parser.
struct MdBlock {
	/// @brief Active alternative. Inspect with `std::get_if<T>(&block.data)` or `std::visit`.
	std::variant<MdHeading, MdParagraph, MdCodeBlock, MdImage, MdHRule, MdTable, MdList, MdBlockQuote> data;
};

/// @brief Sequence of top-level blocks parsed from a Markdown source.
class MarkdownDocument final {
public:
	/// @brief Construct an empty document. Use `parse()` to populate.
	...
	/**
	 * @brief Parse the given Markdown source via md4c.
	...
	/// @brief Top-level blocks (read-only view).
	[[nodiscard]] auto blocks() const -> const std::vector<MdBlock>& { return m_blocks; }

	/// @brief Take ownership of the parsed blocks (clears the document).
