...
/**
 * @brief Thread-safe progress state shared between a background task and the main thread.
 */
...
/**
 * @brief Modal ImGui panel displaying async operation progress.
 */
...
public:
	...
	/**
	 * @brief Start showing the progress modal.
	...
	/// @brief Render the modal. Call every frame from onImGuiRender().
	void onImGuiRender();

	/// @brief Check if the modal is currently active.
	[[nodiscard]] auto isActive() const -> bool { return m_active; }

	/// @brief Check if the operation has finished (success or error).
	[[nodiscard]] auto isFinished() const -> bool;

	/// @brief Close and reset the modal state.
	void close();
